...
if has_key(plugs, 'coc.nvim')
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c      = { 'name' : '+code' }                  | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['a'] = 'LSP Execute code action'             | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['d'] = 'Jump to definition'                  | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['D'] = 'Jump to references'                  | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['f'] = 'Format buffer/region'                | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['i'] = 'Find implementations'                | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['j'] = 'Jump to symbol in current workspace' | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['j'] = 'Jump to symbol in any workspace'     | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['k'] = 'Jump to documentation'               | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['x'] = 'List errors'                         | endif
    if has_key(plugs, 'vim-which-key') | let g:which_key_map.c['t'] = 'Find type definition'                | endif
